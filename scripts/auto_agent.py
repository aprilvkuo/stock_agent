...
def parse_stock_output(output, stock_code):
    ...
    patterns = {
        ...
        'pb': r'市净率.*?([\d.]+)\s*倍',
        'roe': r'ROE.*?([\d.]+)%',
        'revenue': r'营收.*?¥([\d.]+)\s*亿元.*?\+([\d.]+)%',
        'profit': r'净利润.*?¥([\d.]+)\s*亿元.*?\+([\d.]+)%',
        'ma5': r'MA5.*?¥([\d.]+)',
        'ma20': r'MA20.*?¥([\d.]+)',
        'ma60': r'MA60.*?¥([\d.]+)',
        'rsi': r'RSI.*?(\d+)',
...
EXECUTION_LOG_DIR = os.path.join(STOCK_SYSTEM, 'execution-logs')

def log_execution(task_name, target, duration, status, details=None):
    """
    记录任务执行日志（v1.2 新增）
    
    Args:
        task_name: 任务名称 (analyze_stock, daily_validation, weekly_review)
        target: 目标对象 (股票代码或日期)
        duration: 执行耗时 (秒)
    ...
    log_file = os.path.join(EXECUTION_LOG_DIR, f'{today}.jsonl')
    
    with open(log_file, 'a', encoding='utf-8') as f:
        f.write(json.dumps(log_entry, ensure_ascii=False) + '\n')
    
    # 如果耗时超过阈值，打印警告
    if task_name == 'analyze_stock' and duration > 120:
        print(f"⚠️ 警告：{task_name} 耗时过长 ({duration:.2f}秒 > 120 秒)")
    elif task_name == 'daily_validation' and duration > 300:
        print(f"⚠️ 警告：{task_name} 耗时过长 ({duration:.2f}秒 > 300 秒)")
    elif task_name == 'weekly_review' and duration > 600:
        print(f"⚠️ 警告：{task_name} 耗时过长 ({duration:.2f}秒 > 600 秒)")

# ============ 文件操作 ============

def create_analysis_log(stock_code, data, fundamental, technical, sentiment, decision, timing=None):
    """创建分析日志（Markdown + JSON 双格式）v1.4 耗时追踪增强"""
    today = datetime.now().strftime('%Y-%m-%d')
...
| 阶段 | 耗时 |
|------|------|
| 数据获取 | {timing.get('data_fetch', 0):.2f}秒 |
| Agent 分析 | {timing.get('agent_analysis', 0):.2f}秒 |
| 综合决策 | {timing.get('decision', 0):.2f}秒 |
| 日志写入 | {timing.get('log_write', 0):.2f}秒 |
...
def analyze_stock(stock_code):
    ...
    if not data or not data.get('price'):
        print(f"❌ 获取数据失败")
        return False
    print(f"✅ {data.get('stock_name')} 当前价：¥{data['price']} ({data.get('change_pct', 0):+.2f}%) (耗时：{step1_elapsed:.2f}秒)")
    ...
    print(f"   ⚡ 并发分析完成（耗时：{step2_elapsed:.2f}秒）")
    ...
    log_path_md, log_path_json = create_analysis_log(
        stock_code, data, fundamental, technical, sentiment, decision,
        timing={
            'total': total_elapsed,
            'data_fetch': step1_elapsed,
            'agent_analysis': step2_elapsed,
            'decision': step3_elapsed,
        }
    )
    ...
    print(f"✅ 已添加 2 项待验证预测 (耗时：{step5_elapsed:.2f}秒)")
    ...
    print(f"✅ 已更新 (耗时：{step6_elapsed:.2f}秒)")
    ...
    if git_record:
        print(f"✅ Git 提交：{git_record['hash'][:8]} (耗时：{step7_elapsed:.2f}秒)")
    else:
    ...
    print(f"   数据获取：{step1_elapsed:.2f}秒 | Agent 分析：{step2_elapsed:.2f}秒 | 决策：{step3_elapsed:.2f}秒")
    print(f"   日志写入：{step4_elapsed:.2f}秒 | 验证队列：{step5_elapsed:.2f}秒 | 表现追踪：{step6_elapsed:.2f}秒 | Git: {step7_elapsed:.2f}秒")
    print(f"{'='*60}\n")
    
    # 记录到执行日志
    log_execution('analyze_stock', stock_code, total_elapsed, 'success')
...
def run_daily_validation():
    ...
    if result.returncode != 0:
        print(f"❌ 验证执行失败：{result.stderr}")
        log_execution('daily_validation', datetime.now().strftime('%Y-%m-%d'), total_start, 'failed', {'error': result.stderr})
        return False
    ...
    step2_elapsed = time.time() - step2_start
    if git_record:
        print(f"✅ Git 提交：{git_record['hash'][:8]} (耗时：{step2_elapsed:.2f}秒)")
    else:
        print(f"⚠️ Git 提交跳过（无变更或失败）")
    ...
    print(f"✅ 每日验证完成！")
    print(f"   总耗时：{total_elapsed:.2f}秒")
    print(f"   验证执行：{step1_elapsed:.2f}秒 | Git 提交：{step2_elapsed:.2f}秒")
    print(f"{'='*60}\n")
    
    # 记录到执行日志
    log_execution('daily_validation', today, total_elapsed, 'success', {
        'validation_time': step1_elapsed,
        'git_time': step2_elapsed
    })
...
"""
    ...
    step5_elapsed = time.time() - step5_start
    if git_record:
        print(f"✅ Git 提交：{git_record['hash'][:8]} (耗时：{step5_elapsed:.2f}秒)")
    else:
        print(f"⚠️ Git 提交跳过（无变更或失败）")
    ...
    print(f"✅ 周度复盘完成！")
    print(f"   总耗时：{total_elapsed:.2f}秒")
    print(f"   统计记录：{step1_elapsed:.2f}秒 | 验证结果：{step2_elapsed:.2f}秒 | Agent 表现：{step3_elapsed:.2f}秒")
    print(f"   报告生成：{step4_elapsed:.2f}秒 | Git 提交：{step5_elapsed:.2f}秒")
    print(f"{'='*60}\n")
    
    # 记录到执行日志
    log_execution('weekly_review', f'{week_start.strftime("%Y-%m-%d")}_to_{today.strftime("%Y-%m-%d")}', total_elapsed, 'success', {
        'analysis_count': analysis_count,
        'validated_count': validated_count,
        'accuracy': accuracy,
        'steps': {
            'stats': step1_elapsed,
            'validation': step2_elapsed,
            'agents': step3_elapsed,
            'report': step4_elapsed,
            'git': step5_elapsed
        }
    })
