...
# 配置：仅从系统环境变量读取
# =============================================================================
# 必需：GitHub Token (系统环境变量)
GITHUB_TOKEN = os.getenv('GITHUB_BOT_TOKEN') or os.getenv('GITHUB_TOKEN')
if not GITHUB_TOKEN:
    raise RuntimeError(
        "❌ 缺少 GITHUB_TOKEN 环境变量！\n"
        "请设置：export GITHUB_BOT_TOKEN=ghp_xxxxxxxxxxxx\n"
        "或：export GITHUB_TOKEN=ghp_xxxxxxxxxxxx"
    )
...
class GitHubIssueManager:
    ...
    def list_issues(
        ...
        try:
            issues = self._request('GET', 'issues', params=params)
            return issues
    ...
    def create_issue(
        ...
        Args:
            ...
            assignees: 指派给谁（GitHub 用户名列表，默认 ['aprilvkuo']）
            labels: Label 列表
