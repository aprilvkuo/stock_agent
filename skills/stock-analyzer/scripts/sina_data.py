...
def fetch_sina_stock_data(stock_code: str) -> Optional[Dict[str, Any]]:
    """
    从新浪财经获取股票实时数据
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as response:
            content = response.read().decode('gbk')  # 新浪返回 GBK 编码
            
        # 解析数据
        return parse_sina_response(content, stock_code)
...
def parse_sina_response(content: str, stock_code: str) -> Optional[Dict[str, Any]]:
    """
    解析新浪财经返回的数据
    ...
    try:
        ...
        data = {
            ...
            'bid_price': float(fields[6]) if fields[6] else 0,  # 买一价
            'ask_price': float(fields[7]) if fields[7] else 0,  # 卖一价
            'volume': int(float(fields[8])) if fields[8] else 0,  # 成交量（股）
            'turnover': float(fields[9]) if fields[9] else 0,  # 成交额（元）
            'date': fields[28] if len(fields) > 28 else '',
            'time': fields[29] if len(fields) > 29 else '',
            'pe_ttm': float(fields[30]) if len(fields) > 30 and fields[30] else None,  # 动态市盈率
            'pb': float(fields[31]) if len(fields) > 31 and fields[31] else None,  # 市净率
            'turnover_rate': float(fields[32]) if len(fields) > 32 and fields[32] else 0,  # 换手率
            'market_cap': float(fields[33]) if len(fields) > 33 and fields[33] else 0,  # 总市值
            'float_market_cap': float(fields[34]) if len(fields) > 34 and fields[34] else 0,  # 流通市值
            'amplitude': float(fields[35]) if len(fields) > 35 and fields[35] else 0,  # 振幅
            'change_pct': float(fields[36]) if len(fields) > 36 and fields[36] else 0,  # 涨跌幅
            'change': float(fields[37]) if len(fields) > 37 and fields[37] else 0,  # 涨跌额
        }
        
        # 计算涨跌幅（如果 API 未返回）
        if data['change_pct'] == 0 and data['close_yesterday'] > 0:
            data['change_pct'] = (data['current_price'] - data['close_yesterday']) / data['close_yesterday'] * 100
...
def get_stock_data_full(stock_code: str) -> Optional[Dict[str, Any]]:
    ...
    # Step 1: 获取实时行情
    realtime_data = fetch_sina_stock_data(stock_code)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("用法：python sina_data.py <股票代码>")
        print("示例：python sina_data.py 600519")
        sys.exit(1)
    
    stock_code = sys.argv[1]
    
    print(f"\n{'='*60}")
    print(f"测试新浪财经 API - {stock_code}")
    print(f"{'='*60}\n")
    ...
    print(f"【估值指标】")
    print(f"  PE(TTM): {data['pe'] if data['pe'] else 'N/A'}")
    print(f"  PB: {data['pb'] if data['pb'] else 'N/A'}")
    ...
    print(f"  营收：¥{data['revenue']}亿元（+{data['revenue_growth']}%）" if data['revenue'] else "  营收：N/A")
    print(f"  净利润：¥{data['profit']}亿元（+{data['profit_growth']}%）" if data['profit'] else "  净利润：N/A")
    print()
    
    print(f"【技术指标】")
    print(f"  MA5: ¥{data['ma5']}" if data['ma5'] else "  MA5: N/A")
    print(f"  MA20: ¥{data['ma20']}" if data['ma20'] else "  MA20: N/A")
    print(f"  MA60: ¥{data['ma60']}" if data['ma60'] else "  MA60: N/A")
